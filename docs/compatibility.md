...
| `lloadd` behavior | partial | strict bounded raw-BER request envelopes, bidirectional message-ID mapping and final-response ownership, regular/bind pools, round-robin/weighted/best-of scheduling, ordered-tier busy versus unavailable behavior, backend/connection/client limits including bound connections, Simple service/client Bind, ProxyAuthz insertion, auth-only SASL connection pinning, Bind/restriction/write affinity, explicit and disconnect/re-Bind Abandon target rewriting, service-Bind timeout/cancellation, encoded LDAPI upstreams, reconnect/recovery, pinned source contracts, and live OpenLDAP 2.6.13 differentials pass; service Bind without ProxyAuthz, client StartTLS, Cancel, and SASL EXTERNAL are explicitly rejected instead of being forwarded unsafely; client-facing LDAPS/StartTLS/PROXY v2, config-driven upstream TLS and upstream StartTLS/service SASL, SASL WhoAmI identity restoration and security layers, Cancel target rewriting, true isolate-pool semantics, dynamic topology, `cn=config`, `cn=monitor`, and the complete buffering/socket behavior remain |
