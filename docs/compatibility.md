...
slapindex. These are implemented and tested within the boundaries below. They
...
| `lloadd` behavior | partial | bounded BER, pools/scheduling/limits, ProxyAuthz, affinity, Abandon/Cancel, LDAPI, recovery, read backpressure, Verify Credentials, client/upstream TLS, service SASL, keepalive, and TCP user timeout pass. Standalone lifecycle covers OpenLDAP-compatible SIGHUP shutdown, optional SIGUSR1 hot reload, bounded prepare/idle/TLS deadlines, generation drain/Notice, and certificate replacement. The read-only Monitor exposes connection/backend/operation counters with ACL, stable bounded paging, Assertion, Sort, VLV, and ManageDsaIT. PROXY v2 TCP4/TCP6/LOCAL and bounded opaque options pass; PROXY v1, UDP/UNIX PROXY families, full monitor schema/counting, GSSAPI service Bind, embedded module ABI, and complete daemon scheduling parity remain |
...
| LDAP client tools | partial | Built-in search/whoami/compare/passwd/exop/write/delete/modrdn cover simple and implemented SASL mechanisms, LDAP/LDAPS/StartTLS, LDIF, controls, and referrals. `ldapsearch` matches default extended LDIF plus `-L/-LL/-LLL`, repeated `L`, per-query `-c`, `-t/-tt`, paging, client Sort, UFN, SearchReference, known/unknown response controls, and Sync intermediate output. `ldapmodify -a` and omitted-`changetype` Modify, verbose Compare, and OpenLDAP's unreachable Compare `-E` rejection are covered. GSSAPI supports password/FILE keytab/FILE ccache and RFC 4121 security layers; interactive callbacks, platform credential stores, SCRAM-PLUS, complete rebind behavior, and every historical option remain unclaimed |
