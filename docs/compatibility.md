...
| Area | Status | Required evidence |
| --- | --- | --- |
| Transactional durable backend | partial | Memory and bbolt transactionally maintain equality, presence, bounded substring, ordering, and versioned multi-term Metaphone approximate postings across writes, rename, rebuild, backup/restore/check/compact. Empty phonetic terms and unproved matching semantics scan safely. Ordered `default`, `nolang`/`notags`, option-specific AttributeDescription indexes, and `nosubtypes` are represented in the configuration fingerprint and planner. Legacy formats rebuild or scan; final LDAP evaluation remains authoritative. Production-scale selectivity and every custom matching-rule module remain unqualified |
...
response and rolling back cancellable storage work. RFC 3909 Cancel remains
limited to Search; running updates return `cannotCancel`, and pending Search
follows OpenLDAP's `cannotCancel` behavior. Stateless Search/Compare/Who Am I
operations execute concurrently up to a configurable per-connection bound;
stateful requests remain ordered. Update Cancel remains before these rows can
become `compatible`.
