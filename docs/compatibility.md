...
| Web administration | complete | A separate embedded-asset `web-admin` process performs real LDAP Bind and keeps one bound connection per bounded in-memory session, so LDAP ACL/schema/overlay/audit behavior remains authoritative and plaintext passwords are not retained. The responsive UI covers directory tree/filter Search, Person/Group/OU/custom creation, generic attribute modification, Delete, ModifyDN, Password Modify, bounded LDIF import/export, schema and Monitor views. Loopback-only plaintext, HTTPS and LDAP TLS enforcement, opaque HttpOnly SameSite cookies, CSRF plus Origin/Host checks, login limiting, session cleanup, request/search/LDIF bounds, local-file URL rejection, security headers, graceful shutdown, `/livez`, `/readyz`, `/metrics`, fake-client race tests, and a real ldap-go ACL lifecycle pass. Reverse-proxy identity/header trust is intentionally not implemented; terminate TLS directly or proxy only to loopback without trusting forwarded identity headers |
