...
marked `compatible`; planned backends, overlays, operational tooling, native
module ABIs, and protocol edge cases remain. The supported
...
PKCS#8, multi-group TLS configuration, bounded DH/Rand compatibility, and
canonical OpenSSL CA hash validation; semantic ldapsearch response-control and
intermediate-response output plus repeated `-t`; the configured socket-overlay
callback runtime; hardened lloadd daemon generations, signals, Monitor controls
and ACLs; DIGEST-MD5 integrity/privacy layers and a pure-Go GSSAPI
authentication/security layer; passwd, referral-only dnssrv, and asyncmeta
backends; the expanded `{CRYPT}` family including SM3/GOST forms and resource
admission; and slapauth, slapschema, slapmodify, and attribute-selective
...
capabilities, and OpenSSL-versus-Go negotiation and scheduler internals remain
outside the compatibility claim.

The pure-Go build gate covers Linux amd64/arm64, macOS amd64/arm64, Windows
amd64, and FreeBSD amd64. Runtime differentials execute on the host platform;
the build gate is not represented as proof of every platform and provider
combination. FreeBSD explicitly rejects the bundled ODBC connector while
remaining compatible with other registered `database/sql` drivers.
...
| Bind: anonymous and simple | partial | RFC 4511/4513, `olcAllows` LDAPv2/anonymous forms, `olcDisallows`, and OpenLDAP differential tests pass. Password lookup resolves AttributeDescription names, aliases, options, and numeric OIDs through the active schema across Simple Bind, PLAIN/CRAM/DIGEST/SCRAM, ppolicy, remoteauth, translucent old-password checks, and Password Modify; a pinned differential binds and changes a password stored as `2.5.4.35` |
...
| Delete | partial | leaf/subtree/referral behavior tests plus exact Root DSE rejection and connection reuse |
| ModifyDN | partial | rename/move/subtree and RDN-value tests; empty target DN and empty `newSuperior` parse as LDAP DNs and preserve Root DSE result ordering |
| Compare | partial | matching-rule and ACL disclosure tests; Root DSE Compare uses the connection-specific generated entry, frontend restrictions, Compare-before-assertion ACL order, and exact true/false/noSuchAttribute/inappropriateMatching results. A pinned 2.6.13 differential covers all five Root DSE core operation families |
...
| Area | Status | Required evidence |
| --- | --- | --- |
| StartTLS | partial | RFC 4511/4513 state machine plus OpenLDAP `tls_2_anon`/`tls_authc` identity ordering tests pass. A control-free StartTLS request received while another operation is active returns immediate `operationsError` with OpenLDAP's diagnostic instead of waiting behind that operation; a pinned retcode-delayed Search differential passes. Broader control-order and slapd TLS differentials remain |
| Password Modify | partial | RFC 3062 core plus ppolicy old-password, quality, minimum-age, history, reset, hashing, response-control integration, transaction-generated password commit/rollback, and an admin-only one-operation hash-selection control pass; broader cross-overlay cases remain |
| Who Am I? | partial | RFC 4532 simple-bind and StartTLS identity tests pass. The final authorization database's `olcRestrict: extended=1.3.6.1.4.1.4203.1.11.3` is enforced and updates existing bound connections after runtime reload, matching OpenLDAP 2.6.13 |
...
| Matched Values | partial | RFC 3876 BER/control validation, all seven simple item forms, value-level union, equality/substring/ordering/extensible matching, attribute aliases/OIDs/options, ACL intersection, typesOnly/empty-filter behavior, paging stability, Root DSE publication, malformed-BER disconnect, and pinned OpenLDAP 2.6.13 differentials pass. Matching OpenLDAP, unknown/invalid items compute false, approx is inert, user attributes with no matched values are omitted, and empty operational attributes are retained |
| Domain Scope and Search Options | complete | Microsoft/OpenLDAP OIDs `1.2.840.113556.1.4.1339` and `1.2.840.113556.1.4.1340` cover absent/empty/value rules, OpenLDAP-permissive BER, flags 0/1/2/3/-1, criticality, duplicate/combination ordering, hidden Root DSE behavior, continuation-reference suppression, and final referral-to-`noSuchObject` conversion. Local, LDAP proxy, complete-response pCache, chain-compatible frontend filtering, and streaming sock paths pass; pinned OpenLDAP 2.6.13 differentials cover 30 cases |
| Session Tracking | complete | Hidden control `1.3.6.1.4.1.21008.108.63.1` covers all four BER fields, OpenLDAP-permissive inner tags and outer boundaries, exact length limits, non-printable logging behavior, duplicate controls, criticality, operation applicability, and OpenLDAP 2.6.13's invalid-format-OID success/ignore defect. Values remain operation-local, cannot alter ACL/auth identities or trusted peer metadata, are bounded in structured audit output, and pass LDAP proxy client-plus-generated ordering, pCache replay stripping, sock overlay consumption, Root DSE hiding, race, and pinned operation/result differentials |
...
| Tree Delete | complete | Hidden Microsoft/OpenLDAP control `1.2.840.113556.1.4.805` covers absent-value/criticality/duplicate validation and operation applicability. Native storage matches OpenLDAP MDB: critical requests return `unavailableCriticalExtension`, noncritical requests remain ordinary leaf-only Delete. SQL performs full subtree collection, per-entry `entry`/`children` ACL and delete-mapping preflight, deepest-first deletion in one forced transaction even under `olcSqlAutocommit`, No-Op rollback, and base-only Pre-Read/accesslog/audit/sync overlay effects. Pinned MDB/source and live SQLite ODBC back-sql differentials pass. Hardened divergence: OpenLDAP 2.6.13 can return success and commit a deletion prefix after a later ACL denial; ldap-go returns `insufficientAccessRights` and preserves the complete tree |
| Paged results | partial | RFC 2696 Go-client, cookie, ACL, limit, glue, and mutation tests pass. Initial empty-cookie requests with a page size greater than or equal to the LDAP request size limit are ignored as a single unpaged Search, emit no paging response control, and pass critical/noncritical local matrices plus a pinned OpenLDAP 2.6.13 differential |
...
| LDAP transactions (RFC 5805/OpenLDAP) | partial | strict BER, connection state, Add/Modify/Delete/ModifyDN/Password Modify, generated-password commit/rollback, bounded queues, and process-wide retained-byte leases held until End/abort/Bind/Unbind cleanup pass. Transaction Start publishes an admission fence at receipt so pipelined requests cannot be misclassified before state publication. Aborted Transaction Notice, proxy identities, memory/bbolt rollback, OpenLDAP `ldapmodify`, and pinned differentials pass. Transaction Specification remains hidden and critical ProxyAuthz on Start is rejected; broader extension interactions remain |
...
| Subschema subentry | partial | Publishes attribute/object-class and DIT descriptions, 33 implemented public matching rules, native-derived matchingRuleUse, and the supported public ldapSyntaxes subset. Canonical rule-name/OID queries, numeric syntax assertions, operational selection, typesOnly, ACLs, bounded runtime snapshots, and online schema refresh/rollback are covered. Hidden/unimplemented declarations are not fabricated; full native syntax/module inventories remain. See [matching-rule discovery](matching-rule-schema.md) and [syntax discovery](syntax-discovery.md) |
...
| Collective attributes | partial | RFC 3671 schema, propagation, merge/exclusions, filtering, Compare, controls, paging, sorting/VLV, ACLs, X.501 specific/inner administrative areas, autonomous and nested boundaries, role OID/name aliases, and multiple subentries pass. The pinned OpenLDAP 2.6.13 build rejects the tested administrative-role values with `invalidAttributeSyntax`, so ldap-go's X.501 behavior is documented as a standards extension rather than parity evidence |
...
| OpenLDAP ACL grammar and evaluation | partial | filter/value/object-class targets; real/effective DN and level selectors; static/dynamic groups and set expressions; DN/value capture expansion; peer/domain/sockname/sockurl and IPv4/IPv6/path selectors; overall/transport/TLS/SASL SSF; `OpenLDAPaci`; and direct OpenLDAP target, expansion/group, connection/level, and ACI differentials pass. Global `olcReverseLookup` defaults off, snapshots a lowercase PTR hostname at TCP accept, feeds `domain` ACL selectors, ignores resolver failures, and applies online changes only to new connections; deterministic resolver and live ACL tests pass. Unlisted grammar and dynacl modules remain |
...
| Security strength factors | partial | TLS cipher, TLCP SM4, Unix-socket, negotiated DIGEST/GSSAPI layers, minimum SSF, PLAIN policy, and ACL `ssf`/`transport_ssf`/`tls_ssf`/`sasl_ssf` selectors pass. Global `olcLocalSSF` preserves the 71 default, signed-int-to-uint32 conversion, exact online validation, delete reset, and accept-time ldapi snapshot semantics; transport/TLS remain separate and overall SSF is their maximum with SASL. Global and database `olcSecurity` enforce all nine OpenLDAP factors (`ssf`, `transport`, `tls`, `sasl`, four `update_*` factors, and `simple_bind`) with field-wise database override, exact result ordering, and separate transport/TLS/SASL strengths. Global and database `olcRequires` enforce `strong`, `sasl`, `authc`, `bind`, and `LDAPv3` in slapd order across core, proxy, and built-in Extended operations; parser, online rollback, race, and pinned 2.6.13 differentials pass |
...
| Area | Status | Required evidence |
| --- | --- | --- |
| Transactional durable backend | partial | Memory and bbolt use versioned schema-normalized `dn:v2:` physical keys for direct Get/Put/Delete and equality-index maintenance; lookup/replace/delete normalizer work is invariant between small and multi-thousand-entry fixtures. Startup and writable offline tools atomically migrate unmarked legacy partitions after schema load, reject normalized collisions, rebuild postings, and persist per-partition format plus naming-schema fingerprints. Unchanged schema/config publication skips migration in O(1); a changed naming OID/alias/superior/equality fingerprint migrates atomically. Indexed Search performs only an O(1) read-only current check when postings are valid and cannot be blocked by an unrelated bbolt writer; stale state rebuilds under a double-checked database mutex. Markerless legacy routes alone may scan, while `check` rejects marked legacy keys. Reopen, backup, restore, and rebuild preserve the format. A bounded 1k/10k local and 100k nightly qualification records index/paging/mutation/RSS/restart evidence. Every custom matching-rule module and deployment-specific larger scale remain unqualified |
...
| `cn=config` online configuration | partial | OpenLDAP LDIF import, online changes, and immutable runtime publication pass. Generic `X-ORDERED VALUES` schema semantics cover initial Add/slapadd numbering, unindexed append, `{n}` insertion, index-only/content deletion, continuous renumbering, Replace sorting, equality/substring/ordering/approximate matching, permissive modify, and AttributeDescription aliases. `olcAccess`, `olcAuthzRegexp`, and `olcSyncrepl` bootstrap definitions are available before imported config schema publication; a live OpenLDAP 2.6.13 differential matches four online `olcAccess` transitions. `olcSaslCBinding`, per-database `olcMonitoring`, `olcDbMaxEntrySize`, and the four global [logfile settings](logging.md) support startup, online changes, rollback, and restart. Non-default thread controls and LMDB-specific durability/resource settings fail startup, offline validation, and online transactions with rollback; generated OpenLDAP defaults remain importable. Generic `X-ORDERED SIBLINGS` covers fixed negative entries plus database/overlay append, positional insert, Delete renumbering, explicit same-parent/same-type ModifyDN reorder, no-op success, protected config/type/superior errors, descendant subtree moves, canonical DN display, attribute renumbering, Sync change generation, and transaction rollback; live differentials cover database generations and attached syncprov overlays. Local database configuration ModifyDN requires `entryUUID`; legacy entries without it fail closed before mutation so a name-derived content partition cannot become invisible. Complete backend/module-specific validation hooks remain |
| OpenLDAP module inventory | partial | A pinned 2.6.13 source-contract ledger accounts for 15/16 backends, all 25 configured standard overlays, additional chain/pbind/sock/glue/totp runtimes, core Argon2 and six contrib password modules, and all 32 top-level contrib module directories. Unknown/native C modules fail startup, offline validation, and online changes atomically. `back-perl`, `distproc`, SLAPI, native ACL/schema/matching/PAM/NSS/Kerberos/exop contrib modules, and arbitrary third-party ABI modules remain explicitly unsupported; see [openldap-module-coverage.md](openldap-module-coverage.md) |
| `slapcat` content LDIF import/export | partial | semantic round trips, atomic rollback including API dry-run, default/explicit database selection and no-database rejection, glue-subordinate import/export/replacement and `-g`, `config`/`mdb`/`ldif`/`wt`/`null` callback policy, proxy-backend rejection, built-in and supported imported schema validation, ordered `olcLdapSyntaxes`/`X-SUBST`, AttributeDescription/options/`;binary` checks, selected default equality normalizers, RDN-value completion, `structuralObjectClass`, selected-database LastMod, `-S`, root/subentry/config `contextCSN`, selected-database suffix/final-parent checks, and supplied operational metadata pass. Native `import` now detects any config write/clear and performs hierarchy plus runnable-runtime validation before the same transaction commits, without revalidating unchanged config for content-only imports; complete slapd parser/normalizer behavior, executable custom syntax and matching-rule modules, diagnostics, native backend file layouts, and every OpenLDAP data shape remain unverified |
| `slapcat` `cn=config` import | partial | real OpenLDAP 2.6.13 `slapcat -n 0` schema/config import and same-transaction hierarchy/schema/runtime validation pass through both `slapadd -n 0` and native `import`; unsupported configuration modules and behavior-bearing values reject atomically. Generated default and physical MDB metadata remain importable, but bbolt does not treat `olcDbMaxSize` or `olcDbDirectory` as a storage quota/path contract |
| Backup, restore, database rebuild, check | partial | bbolt page/bucket/key/JSON/DN validation, multi-partition metadata-preserving snapshots, online backup, cancellation, atomic publication, fsync and fault injection pass. Offline restore snapshots into private staging, then validates runtime `cn=config`, full entry schema/RDN/structural classes, metadata, allowed partitions, DN identity markers, and complete index postings before exact checked bytes may replace a target. Invalid runtime/schema/index/metadata/orphan-partition candidates leave existing targets byte-identical. `serve -online-backup-dir` exposes the root-authorized LDAPI extension with server-generated names and busy rejection. `backup-prune` provides deterministic dry-run/apply retention by count and age without opening the active database; it confines deletion to strict generated names in an owner-private directory, rejects links/special files/replacement races, and explicitly protects the active database identity. External timers provide scheduling. Windows ACL-safe pruning, remote object storage, and complete filesystem/provider fault matrices remain |
| Monitor backend | partial | Root DSE discovery, all 13 standard monitor branches, dynamic connection/operation/statistics/time/database/overlay state, per-database `olcMonitoring`, and an ldap-go replication branch with stable RID/partition identities plus provider/state/retry/degraded-time/error/cookie-digest health pass Search/Compare/ACL/paging/limits and OpenLDAP 2.6.13 differentials. Memory and bbolt maintain transactionally consistent O(1) partition entry counters; denied, `1.1`, types-only, filtered, and result-limited requests avoid hidden full scans. Complete backend/overlay inventories, LMDB page/reader statistics, and exact OpenLDAP worker internals remain |
...
| Relay backend | partial | Explicit and suffix-massage-selected local targets, bidirectional storage views, Bind/Search/Compare/writes/transactions, ACL translation, inherited sort and sync-provider configuration, direct attribute/objectClass/DN-value mappings, and common librewrite engine/context/rule execution pass focused and OpenLDAP differential tests. Rewritten ordinary and transactional writes enforce original relay restrictions before delegation. POSIX basic regex, external/legacy maps, session variables, relay chains, and broader overlay combinations remain |
| LDAP and meta proxy backends | partial | `ldap` forwards Bind/Search/Compare/writes/Password Modify/Dynamic Refresh/Who Am I with failover, reconnect, identity assertion, proxy authorization, cancellation, diagnostics, referrals, and online rollback. Remote Search entries enforce database/global `olcAccess` or `Config.AccessPolicy` entry/attribute/value visibility with ProxyAuthz, group/ACI auxiliary lookups, policy-keyed pcache, online lifecycle, paging/Sort/referral preservation, bounded failure, and one-upstream-Search ordinary/complete-entry paths; a live OpenLDAP differential pins its response-only ACL behavior and Compare/write bypass. Direct `ldap+rwm` applies suffix, attribute/objectClass, DN/nameAndOptionalUID/URL, filter, assertion, and common librewrite context/rule mappings across Bind/Search/Compare/writes/Password Modify with online rollback. `meta` adds multi-target routing/union, suffix/maps/rules, namespace rewriting, client-pr/onerr/quarantine/DN cache, pooled connection categories, controls, and dynamic target isolation. Meta/asyncmeta ACL evaluation requests hidden target-filter/dnattr/ACI/group/set dependencies, evaluates complete logical entries, then restores the client's projection; no-ACL requests remain unchanged and auxiliary failures fail closed. GSSAPI proxy Bind performs AP-REQ/AP-REP plus RFC 4752 no-layer/integrity/confidentiality on the reused transport, subtracts external TLS SSF exactly as Cyrus SASL does, bounds non-cancellable credential initialization to 16 workers, and isolates pools by resolved credential source. Channel binding defaults to NULL and supports explicit `tls-server-end-point` on verified TLS; fixed Kerberos vectors pass, while repeatable real-KDC automation remains external. Complete referral rebind, POSIX BRE/external rewrite maps/session variables, every connection category, unsafe active topology mutation, target deletion/reordering, other local/frontend overlay execution, and transactions remain |
| Asyncmeta backend | partial | official 2.6.13 schema, multi-target routing/Search union, fixed metaconn scheduling, per-slot pending limits, target connection caps, target-wide timeout retirement including partial-response timeouts, Cancel/Abandon, reload and rollback pass local topology/race tests. Internal queue and connection scheduling is not claimed byte-for-byte equal to OpenLDAP |
| Passwd and DNS-SRV backends | partial | passwd provides bounded read-only ACL-filtered Search and safe inode/permission/mtime refresh. dnssrv returns SRV referrals only, rejects password Bind before DNS, never proxies credentials/writes, and uses bounded TTL/LRU/single-flight caching. Platform account APIs and resolver-provided RR TTL details remain environment-dependent |
| Socket backend and overlay | partial | back-sock operation coverage, bounded parsing, frontend validation, cancellation, reload, controls, and transaction rejection pass. The separate `olcOvSocketConfig` runtime loads masks, DN patterns, connection extensions, paths, and a bounded callback timeout; operation callbacks support `CONTINUE`/direct `RESULT`, while Search sends ordered `ENTRY` and final `RESULT` notifications across multiple instances. SASL Bind and text-protocol fields that cannot represent referrals, response controls, or extended values fail closed; exact POSIX ERE locale and every callback ordering combination remain |
| SQL backend (`back-sql`) | partial | Registers the OpenLDAP 2.6.13 schema, uses `database/sql`/ODBC, and loads standard/custom mappings. Mapped Bind/Search/Compare and transactional Add/Modify/Delete/leaf-ModifyDN plus Tree Delete cover schema, ACLs, controls, binary values, inherited classes, deterministic operational attributes, No-Op, and rollback boundaries. Candidate planning is parameterized and final LDAP evaluation remains authoritative; read views pin one connection/transaction and collective metadata scans use the same transaction without user candidate pruning. Attribute fetch pruning, synthetic or bounded-file base objects, `identity`/single-`suffixmassage` layers, injection-safe scope templates, subtree shortcuts, and `olcSqlCheckSchema` with structural-subclass acceptance pass pure-Go SQLite tests. Pinned OpenLDAP 2.6.13 SQLite ODBC lifecycle and Tree Delete differentials pass without Oracle, but do not establish every directive across drivers. Cross-SQL writes fail before DML because XA is unavailable; a later local-store failure can still follow SQL commit, and RFC 5805 is rejected. Non-SQLite matrices, arbitrary native layer plugins, dynamic operational metadata, and the external Perl adapter remain unsupported or unverified |
...
| allop (contrib) | partial | frontend singleton default mode augments Root DSE base Search with `+` only when user attributes are requested and operational attributes were not already requested; default, `1.1`, and operational-only projections pass a dynamically built pinned `allop.la` differential. The legacy `allop-URI` directive has no `cn=config` attribute and remains outside LDIF configuration import |
| lastbind (contrib) | partial | database singleton `olcLastBindConfig`, `olcLastBindForwardUpdates`, database `olcLastBindPrecision`, successful Simple and SASL Bind, failure exclusion, precision suppression, non-replicated `authTimestamp`, local shadow behavior, and chain-forward change generation pass local/race tests. A dynamically built pinned `lastbind.la` differential verifies first update and precision; full shadow-provider forwarding topology remains |
...
| autoca | partial | Built-in OpenLDAP 2.6.13 PKI/config schema, database-local lifecycle, CA DER reuse, ordered Search issuance, user/server RSA, SANs, PKCS#8, ACLs, concurrency, and pinned semantic differential pass. RSA `olcAutoCAlocalDN` now atomically generates/reuses a validated listener identity for StartTLS/LDAPS when explicit TLS material is absent; metadata failure/reload rollback, restart reuse, concurrent first start, rotation on identity/authority change, and non-disclosure of the listener private key pass. Explicit TLS/TLCP wins. The ldap-go SM2-SM3 issuance extension remains, but automatic SM2 listener installation rejects in favor of explicit TLCP material. Legacy 512-bit RSA, exact OpenSSL extensions, replication/cross-overlay matrices, and explicit CA rotation tooling remain |
| chain | partial | imported `olcChainConfig`/child back-ldap entries; Search continuation, Compare, writes, Password Modify, Dynamic Refresh, identity assertion, TLS/TLCP, chaining behavior, schema/filter policies, client Session Tracking preservation plus ordered proxy-generated tracking, nested referrals, and ppolicy forwarding pass; connection pooling/quarantine scheduling, full SASL rebind coverage, and broader OpenLDAP differential fixtures remain |
...
| noopsrch (contrib) | partial | database/frontend singleton configuration, global control registration, absent-value and duplicate validation, entry/reference suppression, unrestricted counting, and original size-limit reporting pass raw BER tests. A dynamically built pinned `noopsrch.la` differential covers unlimited and client-limited searches; paging, delegated backends, glue, and arbitrary response-overlay ordering remain |
| nops (contrib) | partial | database singleton removes byte-identical non-empty Replace sub-operations under seqmod serialization; all-no-op requests return success without ACL/backend/LastMod/Sync writes, mixed requests retain non-idempotent changes, and the upstream duplicate-value matching quirk is preserved. A dynamically built pinned `nops.la` differential verifies unchanged LastMod; transaction and arbitrary overlay-order combinations remain |
...
| pbind | partial | Database-local Simple Bind forwarding, ordered multi-endpoint failover, StartTLS/TLS parameters, controls, remote result/referral/response-control propagation, quarantine, and OpenLDAP differential pass. Each provider gets an independent network-timeout budget; transient disconnect/`unavailable` retries once before ordered failover, deterministic results do not retry, cancellation closes the upstream, and a process-wide 16-attempt admission bound spans runtime generations. Reload retires old waiters and active transports. Connections remain deliberately one-shot: pooling user-bound transports has no reuse value here and risks identity leakage. Broader TLS platform failures and non-Simple mechanisms remain |
| remoteauth | partial | Ordered mappings, realms/providers, retries, store-on-success, TLS/pins, local precedence, hash writeback and OpenLDAP delegation/writeback/provider-loss differential pass. User-bound transports remain intentionally one-shot; provider/realm/TLS groups get a 16-attempt bound, one-minute lifetime, cancellation closes the socket, runtime reload retires old dialers/in-flight attempts, and secrets are cloned/cleared per attempt. Runtime publication and password writeback share a commit lock, preventing retired configurations from persisting credentials. Idle pooling is deliberately absent because safe anonymous/service reset cannot be proven. A broader TLS platform matrix remains |
| pcache | partial | Database-local `olcPcacheConfig` covers canonical caching, validation, Bind verifiers, reload safety, and pinned Phase 1/2 differentials. Persistence v2 uses immutable candidates, cancellable serialization, generation+epoch CAS, guard tombstones, and retired fingerprints; stale generation-0 writers cannot revive state and slow mutation/encoding/I/O stays outside the read lock. Database roots can Search/Compare and execute Add/Modify/Delete/ModifyDN against the private view; query-only entries are materialized before mutation, and shared responses, counts, UUIDs/URLs and persistent manual entries reconcile atomically with controls, No-Op, cancellation, rollback, and restart. Runtime writes and restored snapshots receive full schema/RDN validation, metadata and entry secret copies are cleared, and large immutable clones occur outside the state lock. Operational metadata is protected. Private Bind frontend ordering and identity reset pass a live OpenLDAP eight-case differential; arbitrary overlay ordering remains unclaimed |
...
| rwm | partial | One- and two-argument `rwm-suffixmassage`, direct and wildcard/drop attribute/objectClass mappings, DN/nameAndOptionalUID/LDAP-URL value translation, filter/assertion translation, and relay/back-meta/direct-back-ldap differentials pass. Relay, back-ldap, and back-meta execute common `rewriteEngine`/`rewriteContext`/`rewriteRule` directives with POSIX captures, ordered flags, bounded recursion, operation variables, parameters, and subcontext calls; transaction replay and online rollback are covered. Local content databases reject named operation rules they cannot execute. POSIX basic regex, external/legacy maps, session variables, normalization controls, regex extensions, and remaining map flags remain unsupported |
...
| Delta-syncrepl | partial | Single-provider accesslog replay, durable cookies, refresh fallback, purge recovery, and Add/Modify/Password Modify/ModDN/Delete cascades pass. Writable multi-provider Modify replay adds attribute-level history merging, schema equality, original-operation forwarding, atomic context/cookies, cross-RID deduplication, and fail-closed history boundaries. Full-suffix ring/mesh and restart coverage uses memory and bbolt. Delete/rename conflicts, increment, ordered values, request controls, filtered replication, and incomplete logging remain unsupported. DSEE changelog coverage uses a protocol-level fake provider; real Oracle DSEE remains unverified |
| Multi-provider and mirror mode | partial | `olcServerID`, writable `olcMultiProvider`/`olcMirrorMode`, durable UUID tombstones, standard three-node convergence, and OpenLDAP bidirectional topology pass. Writable delta supports attribute-level Modify merging with complete local accesslogs; unsupported replay and history gaps preserve cookies and fail closed. Broader failure matrices remain |
...
| Dynamic logging and runtime limits | partial | `cn=Log,cn=Monitor` and global `olcLogLevel` support OpenLDAP names/numeric masks including ANY/NONE and atomically route mapped structured events across ACL/BER/CONFIG/CONNS/STATS/SHELL/SYNC with an `openldap_category` field. Startup, online replacement/deletion, invalid rollback, concurrent routing, persisted values, and OpenLDAP result codes pass pinned source/network differentials. Database `readOnly`, restrictions, inherited `olcSizeLimit`/`olcTimeLimit`, identity-specific soft/hard, unchecked candidates, page/page-total/noEstimate, root behavior, delegated fail-closed handling, and control error ordering pass. Global `olcConnMaxPending`/`olcConnMaxPendingAuth` retain the 100/1,000 defaults, signed base-0 `ARG_INT` parsing, independent fallback, runtime rebuild/rollback, existing-connection publication, identity-specific selection, pending-Abandon quota release, and silent overflow disconnect. `olcSockbufMaxIncoming`/`olcSockbufMaxIncomingAuth` enforce the source 262143/16777215 BER-content defaults before body allocation, unsigned base-0 startup and canonical online values, zero-as-unlimited, exact boundaries, silent overflow, and connection-local anonymous/authenticated snapshots across Bind and online changes. Global `olcIdleTimeout`/`olcWriteTimeout` cover signed base-0 startup values, canonical online Integer rules, atomic rollback, live enable/disable, anonymous/authenticated idle closure, partial BER activity, executing-operation and StartTLS behavior, progress-reset write deadlines, silent slow-reader closure, and Monitor writer state. `olcMaxFilterDepth` defaults to 1,000, preserves signed-int and N/N+1 semantics including negative values, applies online to existing connections after each complete BER frame, covers Search and Assertion controls but not Matched Values, and emits the exact protocolError Notice of Disconnection before closing. Live OpenLDAP 2.6.13 differentials pin pending overflow, PDU limits/identity transitions, filter-depth disconnects, idle timing, 2 MiB blocked responses, and online result codes. Unmapped OpenLDAP debug internals remain |
| Global resource admission | partial | Defaults bound connections (4,096), operations (256), handshakes (64), per-connection stateless execution (8), decoded operation retention (64 MiB connection/256 MiB process including decoded heap structures), retained Search memory (64 MiB operation/512 MiB process including sorted paging/VLV cross-request state), sorted/VLV count (100,000), finite Search totals (128 MiB), single Search PDUs (16 MiB), and in-flight Search writes (256 MiB). Search entry size is checked and process memory reserved before BER allocation; Persistent Sync is exempt only from the finite total. Queue/transaction/state lease release, malformed envelopes, concurrent response reservation, Abandon recheck, local commit-point and remote-dispatch Cancel boundaries, overflow results, and connection reuse are race-tested. Transaction admission is fenced at receipt; Monitor exposes hard/current/rejected byte values. Dynamic online limits and large-scale RSS qualification remain |
...
| `slapadd` / `slapcat` equivalents | partial | `slapadd` supports `-l/-b/-n/-c/-g/-j/-q/-s/-u/-S/-w` and `-o schema-check=yes\|no` / `value-check=yes\|no`; `slapcat` supports `-l/-a/-b/-H/-n/-g/-s` for the tested subset. `slapadd -j` resumes at a physical LDIF line by skipping every complete record whose starting line is lower, including malformed pre-resume records; atomic and `-c` paths plus a pinned OpenLDAP 2.6.13 differential pass. `slapcat -a` compiles a full LDAP filter and evaluates it with the imported schema before combining it with `-s`. `-H ldap:///base??scope?(filter)` supports base/one/sub selection, percent decoding, and the OpenLDAP restriction against hosts, attribute lists, and extensions; a pinned differential matches selected DNs. Default-primary and explicit selection, glue-subordinate import/export, backend callback policy, schema/options/normalizer, LastMod/CSN, root and `olcSyncUseSubentry` context, atomic `cn=config`, dry-run, round-trip, and exit-code cases pass. Without `-c`, import and the direct API remain atomic. Content `-c` depth-batches records, retries a failed batch individually, retains successes, reports record failures, exits nonzero on partial failure, supports disposable `-c -u`, and rejects `cn=config`. `-q` disables value checking only; schema checking remains independently controlled by `-s` / `schema-check=no`, and `objectClass` remains required. Parsing, routing, hierarchy, and storage consistency still run, and quick mode warns and disables explicit `value-check=yes`. Exact OpenLDAP batching/diagnostics, native backend-file behavior, broader `-c` dependency cases, and historical-option parity remain unsupported |
...
| `slaptest` / `slapdn` / `slapauth` / `slapacl` / `slapschema` equivalents | partial | Strict read-only database/config/schema validation, normalized/pretty DN output, multi-DN handling, SASL identity mapping/authorization, and schema scans pass. `slapschema -H` shares slapcat's base/one/sub/filter URL selector. `slapacl` supports real and `-u` synthetic targets, `-D`/`-U` authentication identities, `-X`/`authzDN` authorization identities, `attr[/access][:value]`, effective masks, rootDN, group/set/ACI rules, peer/domain/socket selectors, all four SSFs, password redaction, and read-only bbolt operation. The pure-Go `slapdconf-convert` command expands bounded includes and maps common global/database/TLS/ACL/index/syncrepl/overlay directives to validated `cn=config` LDIF or a new database; unknown behavior directives fail with source locations. Allow/deny and mask output pass a pinned OpenLDAP 2.6.13 differential. Exact diagnostic formatting, every generic process option, and the complete historical `slapd.conf` surface remain |
| `slappasswd` equivalent | partial | Portable digest, SHA-2, PBKDF2, APR1/BSDMD5, SM3/PBKDF2-SM3, TOTP, and bounded `{CRYPT}` generation/verification pass. `{CRYPT}` covers DES/BSDI/bigcrypt, MD5, bcrypt, SHA-256/512, yescrypt/scrypt, SHA1/SunMD5/NT, SM3-crypt, SM3/GOST-yescrypt, controlled `-c`, configurable secure server salt format, and global resource admission. Verify-only Netscape/RADIUS and unsupported provider-specific/high-cost crypt formats fail explicitly |
| LDAP client tools | partial | Built-in search/whoami/compare/passwd/exop/write/delete/modrdn and the VC subset below cover implemented authentication, transports, LDIF, controls, and referrals. General `-e` accepts numeric OIDs plus OpenLDAP `assert`, `authzid`, `manageDSAit`, `noop`, `ppolicy`, `preread`, `postread`, `relax`/`manageDIT`, and `sessiontracking`; name/OID duplicate detection, exact BER/criticality, default identity selection, and named ppolicy/session-tracking controls on simple and SASL Bind pass a live OpenLDAP 2.6.13 command-wire differential. Common `-o nettimeout=<seconds\|none\|max>` syntax configures every direct, observed, referral, StartTLS, TLS, and SASL dial/deadline path, with duplicate/conflict/invalid-value checks. `ldapsearch` matches extended LDIF suppression, repeated `-t`, paging/Sort, UFN, references, response controls, Sync output, `-M/-MM` ManageDsaIT, named `-E [!]sss=[-]attr[:rule][/...]`, one-window `vlv=before/after/(offset/count|:value)`, `deref=attr:attrs`, `subentries`, `sync=ro|rp[/cookie][/<slimit>]`, critical `dontUseCopy`, `accountUsability`, `domainScope`, `mv=<filter>`, and explicit RFC 4516 direct URL queries. RefreshAndPersist is an immediate, memory-bounded stream across Simple/SASL/LDAPS/StartTLS and uses RFC 3909 Cancel at the OpenLDAP signed response limit; four LDIF levels match complete OpenLDAP stdout/stderr/exit output. Deref response controls are bounded/decoded and render printable and binary values identically to OpenLDAP. Named controls pass bounded codec, raw-wire, pinned-source, and OpenLDAP CLI differentials; interactive VLV window iteration remains unimplemented. `ldapmodify` covers `-a`, omitted-`changetype` Modify, `-j`, bounded `file://`, record controls, control-preserving `-S`, OpenLDAP-style `-v`, and real `-M/-MM` ManageDsaIT. Verbose Compare and OpenLDAP's unreachable Compare `-E` rejection are covered. GSSAPI supports password/FILE keytab/FILE ccache and RFC 4121 layers. Removed legacy `-h/-p` host/port flags are supported as a fail-closed old-script extension and tested against equivalent 2.6.13 `-H` calls; interactive callbacks, platform stores, complete rebind behavior, and remaining historical options remain unclaimed |
| Web administration | complete | A separate embedded-asset `web-admin` process performs real LDAP Bind and keeps one bound connection per bounded in-memory session, so LDAP ACL/schema/overlay/audit behavior remains authoritative and plaintext passwords are not retained. The responsive English/Simplified Chinese UI covers directory tree/paged/visual-filter Search, local bookmarks/history, Person/POSIX account/Group/POSIX Group/OU/custom creation, schema-aware and binary/image attributes, clone, Delete, ModifyDN, isolated current-password Bind verification gated by self identity or LDAP Compare access to password attributes, Password Modify with optional old-password verification, and curated per-change hash selection through ldap-go's advertised critical Password Modify control. The LDAP server validates optional old credentials, ACL and ppolicy against plaintext before hashing; scheme selection requires password-attribute `manage` access, is unavailable to ordinary self-write users, is bounded to 4 KiB, and never returns the hash in the mutation response. Targets that do not advertise the control receive only normal server-policy Password Modify. It also covers explicit bounded bulk operations; direct/nested group members; paged member selection; reverse DN, NameAndOptionalUID, and UID memberships with shortest-path/cycle reporting; LDIF/CSV import; LDIF/CSV/JSON export; schema; and Monitor. Every write path, including group membership and binary replace/delete, distinguishes deterministic LDAP errors from timeout/EOF/network outcomes whose commit result is unknown; unknown results retire the bound session and reject direct retry. Browser-local language selection never transforms LDAP identifiers or values. Loopback-only plaintext, HTTPS and LDAP TLS enforcement, opaque HttpOnly SameSite cookies, CSRF plus Origin/Host checks, login/credential-verification limiting, session cleanup, request/search/import/export/binary/nesting bounds, local-file URL rejection, security headers, graceful shutdown, `/livez`, `/readyz`, `/metrics`, fake-client race tests, and a real ldap-go ACL lifecycle pass. Multi-server profiles, cross-server copy, scheduled jobs/reports, and proprietary SQLLDAP compatibility are intentionally outside this Web boundary. Reverse-proxy identity/header trust is not implemented; terminate TLS directly or proxy only to loopback without trusting forwarded identity headers |
| Load balancer tooling | partial | Standalone config validation, LDAP/LDAPS/StartTLS/LDAPI/PROXY v1/v2 listeners, upstream TLS/SASL/socket options, signal lifecycle, optional atomic hot reload, drain timeout, and bounded Monitor administration pass unit/race/source tests. DGRAM/UDP PROXY, complete historical logging/signal surface, persistent monitor snapshots, embedded slapd-module mode, and exact worker scheduling remain unsupported |

The lloadd upstream service client also supports all three SCRAM-PLUS hashes
over verified standard LDAPS/StartTLS, with endpoint channel binding, bounded
challenge work, server-proof validation, and pool/certificate-generation
isolation. Unverified TLS, downgrade, TLCP, and other channel-binding types are
...
SCRAM-SHA-1/256/512 and their `-PLUS` variants use a connection-scoped multi-round Bind state and reject
...
OpenLDAP SCRAM-SHA-256 interoperability. PLUS uses `tls-server-end-point`, is
published only on standard TLS connections with available certificate binding
data, rejects TLCP, and passes fixed-vector, local TLS, replay, downgrade, and
native OpenLDAP/Cyrus SHA-1/256/512 tests. Negotiated SASL integrity/privacy
...
Database `olcLimits` applies schema-aware identity selectors to size/time
soft/hard, unchecked candidate, page-size, estimate, and paged-total fields.
`olcSizeLimit`/`olcTimeLimit` provide database defaults and inherit from the
frontend unless overridden. Project `MaxSearchEntries` remains an absolute
safety cap. Local candidate bounds are checked before result emission;
delegated backends with a positive unverifiable unchecked bound fail closed.
Exact/one/subtree/children, POSIX `dn.regex`, `dn.this` variants, and
`group[/objectClass[/attribute]]` selectors operate on normalized DNs. Group
objectClass/member values are schema-aware and use backend-internal visibility,
matching OpenLDAP's `backend_group()` behavior; delegated backends that cannot
prove membership fail closed. Database roots bypass subject rules
while retaining the project safety cap.
...
CSN idempotency, and subtree rename are handled explicitly. For single-provider
replication, a provider `syncRefreshRequired` result or a local replay conflict clears only that
...
Writable `olcMultiProvider`/`olcMirrorMode` accesslog consumers support
attribute-level Modify conflict merging using OpenLDAP 2.6.13's
`mods_dup`/`syncrepl_resolve_cb` rules. Older replacements split into
delete/add, newer values cancel conflicting older changes using schema
equality, and newer entryCSN/modification operational attributes survive.
Data, original accesslog modifications, context, and cookie commit atomically.
Configuration requires local syncprov databases, lastmod, all-write logging,
and unfiltered full-suffix accesslog consumers. Delete/rename, increment,
ordered values, request controls, UUID conflicts, and missing or purged
history fail closed. A writable peer retains its cookie on failure; it does
not fall back to full-entry refresh over existing data.
Initial bootstrap is tracked separately per consumer/database. An explicit
in-progress state survives interruption; Sync Done atomically persists the
completed data identity and final cookie. A markerless writable database with
...
ldap-go additionally advertises critical control
`1.3.6.1.4.1.4203.666.5.20` for an administrator-selected hash on one Password
Modify operation. Its control value is one canonical allow-listed scheme. The
server requires `manage` access to the target password attribute, rejects
ordinary self-write identities and inputs above 4 KiB, verifies any supplied
old password and enforces plaintext password quality and history before doing
the selected hash work. It rejects the private control inside LDAP transactions
and on translucent databases. A server or backend that cannot honor the
critical control rejects the operation without changing the password.
...
response and canceling server-controlled work. RFC 3909 Cancel covers active
Search, Compare, Add, Modify, Delete, ModifyDN, and non-boundary Extended
operations; it emits the original operation's application response with result
`canceled` before acknowledging Cancel. Pending targets retain OpenLDAP's
`cannotCancel` behavior, and Bind/Unbind/Abandon, StartTLS, Who Am I, Cancel,
and transaction Start/End are non-cancelable. Stateless Search/Compare/Who Am I
operations execute concurrently up to a configurable per-connection bound;
stateful requests remain ordered. Remotely delegated writes atomically become
non-cancelable before dispatch; if Cancel wins first, dispatch observes the
canceled context and sends nothing, while dispatch-first returns
`cannotCancel`. Broader delegated-backend cancellation differentials remain
before these rows can become `compatible`.
...
globally sorted top-N entries. Same-connection Search/Compare/Who Am I execution is
...
configuration schema, backend/module-specific mutation hooks, and full
differential error behavior remain pending. `X-ORDERED VALUES` attributes are numbered, inserted, deleted,
renumbered, and matched through the shared schema and mutation paths;
`X-ORDERED SIBLINGS` database/overlay Add and Delete atomically renumber their
