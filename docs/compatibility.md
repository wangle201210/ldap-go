...
| Area | Status | Required evidence |
| --- | --- | --- |
| Transactional durable backend | partial | Memory and bbolt use versioned schema-normalized `dn:v2:` physical keys for direct Get/Put/Delete and equality-index maintenance; lookup/replace/delete normalizer work is invariant between small and multi-thousand-entry fixtures. Startup and writable offline tools atomically migrate unmarked legacy partitions after schema load, reject normalized collisions, rebuild postings, and persist a per-partition marker. Markerless legacy server routes and full iteration alone may scan; a marked partition containing a legacy key fails `check`. Reopen, backup, restore, and rebuild preserve the format. Equality, presence, bounded substring, ordering, and versioned multi-term Metaphone postings remain transactionally maintained. Production-scale selectivity and every custom matching-rule module remain unqualified |
