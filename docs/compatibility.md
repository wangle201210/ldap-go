...
an automated real-KDC interoperability topology, dependency-specific GSSAPI
...
| Bind: SASL | partial | EXTERNAL, PLAIN, CRAM-MD5, SCRAM-SHA-1/256/512 and all three `-PLUS` variants, DIGEST-MD5 `auth`/`auth-int` plus RC4/DES/3DES `auth-conf`, and pure-Go GSSAPI no-layer/integrity/confidentiality pass vector, local, race, SSF, and selected native-client tests. SCRAM-PLUS enforces RFC 5802/5929 `tls-server-end-point` over verified standard TLS, rejects TLCP, downgrade, binding mismatch, and replay, and passes native OpenLDAP 2.6.13/Cyrus tests for all three hashes. DIGEST block ciphers cover Cyrus EDE2/CBC vectors, padding/MAC/sequence/maxbuf and key cleanup; native Cyrus DES passes, while the local Cyrus/OpenSSL 3 3DES provider crashes before its first frame and is recorded as an environment defect. Repeatable real-KDC automation, platform stores, and delegation remain |
