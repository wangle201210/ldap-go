...
| SQL and perl-style adapters | partial | `back-sql` registers the exact OpenLDAP 2.6.13 `olcSqlConfig` schema, opens ODBC DSNs through `database/sql`, loads standard/custom objectClass and attribute mapping queries, and serves mapped Bind/Search/Compare with frontend schema, matching, scope, ACL, controls, binary values, `structuralObjectClass`, `entryUUID`, and `hasSubordinates`; pure-Go SQLite end-to-end tests require no Oracle or external RDBMS. SQL Add/Modify/Delete/ModifyDN transactions, procedure OUT parameters, baseobject/sqllayer, candidate SQL filter optimization, OpenLDAP SQL differential tests, and the external Perl compatibility runner remain; SQL writes currently fail explicitly with `unwillingToPerform` instead of mutating local storage |
