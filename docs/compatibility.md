...
| Bind: SASL | partial | EXTERNAL, PLAIN, CRAM-MD5, SCRAM-SHA-1/256/512, DIGEST-MD5 `auth`/`auth-int`/RC4 `auth-conf`, and pure-Go GSSAPI no-layer/integrity/confidentiality pass vector, local, race, SSF, and selected native-client tests. GSSAPI preserves initial sequence/subkey state and defaults to RFC 4752 NULL channel binding; repeatable real-KDC/OpenLDAP automation, platform credential stores, SCRAM-PLUS, DIGEST DES/3DES, and delegation remain |
...
| SASL server authentication | partial | EXTERNAL, PLAIN, CRAM-MD5, SCRAM-SHA-1/256/512, DIGEST-MD5 `auth`/`auth-int`/`auth-conf` with RC4 variants, and pure-Go GSSAPI no-layer/integrity/confidentiality pass local, vector, race, SSF, and selected native-client tests. GSSAPI preserves context sequence/subkey state and defaults to RFC 4752 NULL channel binding; `tls-server-end-point` is explicit. SCRAM-PLUS, DIGEST DES/3DES, native credential-store breadth, and an automated real-KDC interoperability topology remain |
...
| pcache | partial | Database-local `olcPcacheConfig` on back-ldap covers canonical positive/negative queries, schema-aware containment, TTL/TTR/offline/LRU/limits, paging, validation, head/tail, and non-cleartext Bind caching with reload safety and pinned Phase 1/2 differentials. `olcPcachePersist` atomically stores and restores unexpired query/bind snapshots, including correct UUID transitions across positive/negative refresh. Database roots can Search/Compare the private cache through OID `1.3.6.1.4.1.4203.666.11.9.5.1`; query IDs/URLs and selected queryDelete operations are exposed and advertised in Root DSE. Unsupported private writes return `unwillingToPerform`. Arbitrary overlay ordering and a fully writable private MDB backend are not claimed |
...
| `lloadd` behavior | partial | bounded BER, pools/scheduling/limits, ProxyAuthz, affinity, Abandon/Cancel, LDAPI, recovery, read backpressure, Verify Credentials, client/upstream TLS, service SASL, keepalive, and TCP user timeout pass. Standalone lifecycle covers OpenLDAP-compatible SIGHUP shutdown, optional SIGUSR1 hot reload, bounded deadlines, generation drain/Notice, and certificate replacement. The read-only Monitor exposes connection/backend/operation and single-terminal outcome counters with ACL, stable paging, Assertion, Sort, VLV, and ManageDsaIT; raw BER covers noncritical Sort/VLV failures. Serving Monitor on the data listener is an explicit ldap-go extension because pinned standalone OpenLDAP returns `unavailable`. PROXY v2 TCP4/TCP6/LOCAL passes; PROXY v1, UDP/UNIX PROXY families, full monitor schema, GSSAPI service Bind, embedded ABI, and exact scheduling remain |
...
| `slapmodify` equivalent | partial | Atomic offline Add/Modify/Delete/ModifyDN supports `-j` record resume and `-w` contextCSN updates. Delete operations advance the correct SID even after the entry disappears; multi-SID aggregation and commit-failure rollback pass Memory/bbolt tests. Exact OpenLDAP diagnostics and every historical option remain |
...
| LDAP client tools | partial | Built-in search/whoami/compare/passwd/exop/write/delete/modrdn cover implemented authentication, transports, LDIF, controls, and referrals. `ldapsearch` matches extended LDIF suppression, repeated `-t`, paging/Sort, UFN, references, response controls, and Sync intermediate output. `ldapmodify` covers `-a`, omitted-`changetype` Modify, physical-line `-j`, bounded absolute `file://` values, record controls, and control-preserving `-S` replay. Verbose Compare and OpenLDAP's unreachable Compare `-E` rejection are covered. GSSAPI supports password/FILE keytab/FILE ccache and RFC 4121 layers; interactive callbacks, platform stores, SCRAM-PLUS, complete rebind behavior, and every historical option remain unclaimed |
| Load balancer tooling | partial | Standalone config validation, LDAP/LDAPS/StartTLS/LDAPI/PROXY listeners, upstream TLS/SASL/socket options, SIGHUP lifecycle, optional atomic hot reload, drain timeout, and bounded Monitor administration pass unit/race/source tests. PROXY v1/non-TCP families, complete historical logging/signal surface, persistent monitor snapshots, embedded slapd-module mode, and exact internal worker scheduling remain unsupported |
