...
| rwm | partial | one- and two-argument `rwm-suffixmassage`, direct and wildcard/drop attribute/objectClass mappings, DN/nameAndOptionalUID/LDAP-URL value translation, filter/assertion translation, and relay/back-meta/direct-back-ldap differentials pass. Direct back-ldap covers Bind, Search, Compare, Add, Modify, Delete, ModifyDN, Password Modify, controls, online configuration, and no-RWM fast paths. Unsupported `olcRwmRewrite`/`olcDbRewrite` directives fail startup or atomic online configuration with persistence/runtime rollback instead of being silently ignored; arbitrary rewrite contexts/rules, normalization controls, and remaining map flags remain |
...
| Delta-syncrepl | partial | OpenLDAP and ldap-go single-provider accesslog `reqMod` replay, durable cookies, refresh fallback, purge/minCSN gap recovery, and provider-to-consumer Add/Modify/Password Modify/ModDN/Delete topology pass. A real A-to-B-to-C cascade now records remote operations, original CSNs, source/accesslog context, tombstones, and RID cookies atomically on B, publishes only after commit, and covers cross-RID deduplication, restart, rollback, multi-AVA/suffix-massage ModDN, scope move recovery/delete, and purge boundaries. DSEE `syncdata=changelog` covers Root DSE bounds, initial/gap snapshots, LDIF Add/Modify/ModDN/Delete replay, durable `lastChangeNumber`, and Persistent Search against a protocol-level fake provider. Because attribute-level delta multi-provider conflict merging is not implemented, combining `syncdata=accesslog` with writable `olcMultiProvider`/`olcMirrorMode` fails startup, offline validation, and atomic online Add/Modify instead of risking divergence; real Oracle DSEE and broader failure topologies remain unverified |
| Multi-provider and mirror mode | partial | `olcServerID`, writable non-delta `olcMultiProvider`/`olcMirrorMode`, durable UUID tombstones, three-node relay/restart/offline conflict convergence, and OpenLDAP 2.6 bidirectional topology pass. Delta multi-provider is explicitly rejected until OpenLDAP-compatible attribute-level history merging is implemented; broader failure matrices remain |
