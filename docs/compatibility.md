...
| pcache | partial | Database-local `olcPcacheConfig` on back-ldap covers canonical positive/negative queries, schema-aware containment, TTL/TTR/offline/LRU/limits, paging, validation, head/tail, and non-cleartext Bind caching with reload safety and pinned Phase 1/2 differentials. Persistence v2 atomically stores unexpired query/bind state with immutable candidates, a cancellable serializer, generation+epoch CAS, guard tombstones, and retired-fingerprint protection. Disable/re-enable cannot revive a generation-0 stale writer; slow storage, encoding, and mutation work do not hold the read lock, and failures publish neither memory nor metadata. Database roots can Search/Compare the private cache and delete selected queries. Unsupported private writes return `unwillingToPerform`; arbitrary overlay ordering and a fully writable private MDB backend are not claimed |
...
| `lloadd` behavior | partial | bounded BER, pools/scheduling/limits, ProxyAuthz, affinity, Abandon/Cancel, LDAPI, recovery, backpressure, Verify Credentials, TLS, service SASL, keepalive, and TCP user timeout pass. Lifecycle covers signals, bounded deadlines, generation drain/Notice, and certificate replacement. Monitor exposes connection/backend/operation and single-terminal counters with ACL, paging, Assertion, Sort, VLV, and ManageDsaIT; serving it on the data listener is an explicit extension. Trusted listeners accept strict PROXY v1 TCP4/TCP6/UNKNOWN and v2 TCP4/TCP6/LOCAL/UNIX stream before TLS where applicable. V2 UNIX dispatch is an extension over pinned lloadd; DGRAM/UDP, full monitor schema, GSSAPI service Bind, embedded ABI, and exact scheduling remain |
...
Database roots bypass subject rules while retaining the project safety cap.
