...
| SQL and perl-style adapters | partial | `back-sql` registers the exact OpenLDAP 2.6.13 `olcSqlConfig` schema, opens ODBC DSNs through `database/sql`, adapts standard `sql.Out` values to the godbc procedure-output API, loads standard/custom objectClass and attribute mapping queries, and serves mapped Bind/Search/Compare with frontend schema, matching, scope, ACL, controls, binary values, `structuralObjectClass`, `entryUUID`, and `hasSubordinates`. Add/Modify/Delete/leaf-ModifyDN execute mapping procedures and `ldap_entries` changes in one SQL transaction; No-Op and mapped-procedure failures roll back. `olcSqlAutocommit` uses a pinned connection without an explicit transaction, including OpenLDAP's inability to roll back a No-Op write. Each LDAP read view uses one pinned transaction, preferring a repeatable-read/read-only snapshot and falling back to driver-default transaction options, and startup, offline validation, online `cn=config` changes, failed candidates, and retired pools have eager validation and lifecycle coverage. Pure-Go SQLite end-to-end tests require no Oracle or external RDBMS. A request that would write two SQL backends is rejected because independent databases cannot be atomically committed without XA; sync/accesslog metadata spanning SQL and the local store has the same non-XA limitation. Live ODBC procedure differentials, subtree ModifyDN, baseobject/sqllayer, requested-attribute and candidate-filter SQL optimization, dynamic operational metadata, OpenLDAP SQL differentials, and the external Perl compatibility runner remain |
...
| pcache | partial | database-local `olcPcacheConfig` on back-ldap, canonical/legacy configuration names, structured equality-template matching, attrset-aware keys, positive/negative caching, OpenLDAP's delayed `>` entry boundary, TTL/consistency windows, TTR single-flight refresh, offline expiry pause/resume, query-level LRU, `olcPcacheMaxQueries`, provider-outage hits, requested-attribute/`typesOnly` reprojection, paging policy, concurrent/reload-safe state, intentionally stale proxy writes, and pinned OpenLDAP 2.6.13 Phase 1/2 differentials pass; `olcPcachePersist` is accepted but reproduces the observed no-restore behavior rather than promising durable query recovery; Bind cache, private DB/query deletion, full containment, advanced controls, and broader overlay ordering remain |
...
| Area | Status | Required evidence |
| --- | --- | --- |
| Syncrepl consumer | partial | ordered `olcSyncrepl` loading plus ldap-go, OpenLDAP 2.6.13 standard, accesslog delta, and protocol-level fake DSEE retro changelog initial/persist/restart/recovery topologies pass; tag-based Sync Info compatibility, initial-refresh timeout semantics, and DIGEST-MD5 realm/authzid/server-proof handling pass; real Oracle DSEE and broader provider variants remain unverified |
...
| `lloadd` behavior | partial | strict bounded raw-BER request envelopes, bidirectional message-ID mapping and final-response ownership, regular/bind pools, round-robin/weighted/OpenLDAP-decaying-best-of scheduling, ordered-tier busy versus unavailable behavior, backend/connection/client limits including bound connections, Simple service/client Bind, ProxyAuthz insertion, auth-only SASL connection pinning, Bind/restriction/write affinity, explicit and disconnect/re-Bind Abandon target rewriting, RFC 3909 Cancel on the target's exact upstream with outer/inner ID rewriting, association isolation, ProxyAuthz preservation, bounded capacity bypass and non-abandonable lifecycle, service-Bind timeout/cancellation, encoded LDAPI upstreams, reconnect/recovery, pinned source contracts, and live OpenLDAP 2.6.13 differentials pass; service Bind without ProxyAuthz, client StartTLS, and SASL EXTERNAL are explicitly rejected instead of being forwarded unsafely; OpenLDAP 2.6.13 lloadd does not rewrite Cancel's inner ID and recommends rejecting it, so the Go behavior is an intentional RFC correction rather than bug-for-bug parity; client-facing LDAPS/StartTLS/PROXY v2, config-driven upstream TLS and upstream StartTLS/service SASL, SASL WhoAmI identity restoration and security layers, true isolate-pool semantics, dynamic topology, `cn=config`, `cn=monitor`, and the complete buffering/socket behavior remain |
