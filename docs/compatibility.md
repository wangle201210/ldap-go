...
| LDAP client tools | partial | Built-in search/whoami/compare/passwd/exop/write/delete/modrdn cover implemented authentication, transports, LDIF, controls, and referrals. Common `-o nettimeout=<seconds\|none\|max>` syntax configures every direct, observed, referral, StartTLS, TLS, and SASL dial/deadline path, with duplicate/conflict/invalid-value checks. `ldapsearch` matches extended LDIF suppression, repeated `-t`, paging/Sort, UFN, references, response controls, Sync output, `-M/-MM` ManageDsaIT, named `-E [!]sss=[-]attr[:rule][/...]`, `subentries`, `sync=ro|rp[/cookie]`, critical `dontUseCopy`, `accountUsability`, `domainScope`, `mv=<filter>`, and explicit RFC 4516 direct URL queries. Named controls pass raw-wire and OpenLDAP CLI differentials; refreshAndPersist response-limit auto-cancel remains unimplemented. `ldapmodify` covers `-a`, omitted-`changetype` Modify, `-j`, bounded `file://`, record controls, control-preserving `-S`, OpenLDAP-style `-v`, and real `-M/-MM` ManageDsaIT. Verbose Compare and OpenLDAP's unreachable Compare `-E` rejection are covered. GSSAPI supports password/FILE keytab/FILE ccache and RFC 4121 layers; interactive callbacks, platform stores, SCRAM-PLUS, complete rebind behavior, and every historical option remain unclaimed |
