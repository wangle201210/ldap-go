...
| SQL backend (`back-sql`) | partial | Registers the OpenLDAP 2.6.13 schema, uses `database/sql`/ODBC, and loads standard/custom mappings. Mapped Bind/Search/Compare and transactional Add/Modify/Delete/leaf-ModifyDN cover schema, ACLs, controls, binary values, inherited classes, deterministic operational attributes, No-Op, and rollback boundaries. Candidate planning is parameterized and final LDAP evaluation remains authoritative; read views pin one connection/transaction and collective metadata scans use the same transaction without user candidate pruning. Attribute fetch pruning, synthetic or bounded-file base objects, `identity`/single-`suffixmassage` layers, injection-safe scope templates, subtree shortcuts, and `olcSqlCheckSchema` with structural-subclass acceptance pass pure-Go SQLite tests. A pinned OpenLDAP 2.6.13 SQLite ODBC lifecycle differential passes without Oracle, but does not establish every directive across drivers. Cross-SQL writes fail before DML because XA is unavailable; a later local-store failure can still follow SQL commit, and RFC 5805 is rejected. Non-SQLite matrices, arbitrary native layer plugins, dynamic operational metadata, and the external Perl adapter remain unsupported or unverified |
