...
| TLS and mutual TLS | partial | LDAPS/StartTLS/client cert and syncrepl CA/SAN/CRL policies pass; supported global `olcTLS*` certificate/key/CA, client-verification, minimum-protocol, exact cipher settings, and inert `olcTLSCRLCheck=none` build or stage an immutable runtime context; online TLS enable/disable, LDAPS/StartTLS certificate rotation, existing-connection isolation, invalid-update rollback, and required-client-certificate failure pass; unsupported OpenSSL selectors, global CRL `peer`/`all` enforcement, DH/random/curve directives, TLS 1.3 cipher selection, and the broader slapd TLS matrix remain |
...
| Area | Status | Required evidence |
| --- | --- | --- |
| Transactional durable backend | partial | crash, atomicity, recovery, and race tests pass; schema-aware v2 DN keys lazily replace legacy v1 folded keys on successful writes in Memory and bbolt, preserve caseExact siblings, merge caseIgnore/alias/OID/multi-AVA equivalents, and fail closed on ambiguous legacy/v2 identities; check/backup/restore/compact/rebuild preserve physical records and schema-aware duplicate checks; configurable equality/substring/ordering indexes and indexed Search planning remain unsupported, so local database Search currently scans the selected partition |
...
| pcache | partial | database-local `olcPcacheConfig` on back-ldap, canonical/legacy configuration names, attrset-aware canonical keys, positive/negative caching, OpenLDAP's delayed `>` entry boundary, TTL/consistency windows, TTR single-flight refresh, offline expiry pause/resume, query-level LRU, `olcPcacheMaxQueries`, provider-outage hits, requested-attribute/`typesOnly` reprojection, paging policy, concurrent/reload-safe state, intentionally stale proxy writes, and pinned OpenLDAP 2.6.13 Phase 1/2 differentials pass. Template containment covers schema-aware equality, OpenLDAP-direction substring containment, unordered one-to-one AND/OR matching, attribute/OID and matching-rule aliases, and extensible filters; unknown schema elements fail closed. `olcPcacheBind` caches non-cleartext verifiers after successful remote Bind with DN identity, TTL/offline, entry/query limits, concurrent verification, provider fallback, successful-reload clearing, and failed-candidate preservation. `olcPcachePersist` reproduces the observed no-restore behavior rather than promising durable recovery; private DB/query deletion, advanced controls, broader Bind differentials, and arbitrary overlay ordering remain |
...
| `lloadd` behavior | partial | strict bounded raw-BER request envelopes, bidirectional message-ID mapping and final-response ownership, regular/bind pools, round-robin/weighted/OpenLDAP-decaying-best-of scheduling, ordered-tier busy versus unavailable behavior, backend/connection/client limits including bound connections, Simple service/client Bind, ProxyAuthz insertion, auth-only SASL connection pinning, Bind/restriction/write affinity, explicit and disconnect/re-Bind Abandon target rewriting, RFC 3909 Cancel on the target's exact upstream with outer/inner ID rewriting, association isolation, service-Bind timeout/cancellation, encoded LDAPI upstreams, reconnect/recovery, pinned source contracts, and live OpenLDAP 2.6.13 differentials pass. Client LDAP StartTLS and LDAPS listeners pass real handshakes; upstream LDAPS and optional/critical StartTLS validate CA, SAN/hostname, client certificate, protocol, cipher, curve, and CRL policy before service Bind. Service auth supports Simple plus auth-only PLAIN, CRAM-MD5, DIGEST-MD5, and SCRAM-SHA-1/256/512 with server-proof checks. TCP keepalive is applied to TCP and TLS-wrapped upstreams, Linux `TCP_USER_TIMEOUT` is tested, and LDAPI is excluded. Service Bind without ProxyAuthz and client SASL EXTERNAL remain explicitly rejected; OpenLDAP 2.6.13 lloadd does not rewrite Cancel's inner ID, so the Go rewrite is an intentional RFC correction. GSSAPI, SCRAM-PLUS/channel binding, SASL security layers and WhoAmI identity restoration, client PROXY v2, true isolate-pool semantics, dynamic topology, `cn=config`, `cn=monitor`, and complete buffering/socket/daemon parity remain |
...
| LDAP client tools | partial | `ldapsearch`, `ldapwhoami`, `ldapcompare`, `ldappasswd`, `ldapexop`, `ldapmodify`/`ldapadd`, `ldapdelete`, and `ldapmodrdn` cover simple Bind, LDAP/LDAPS/StartTLS, password prompting/files, LDIF writes, Compare exit codes, extended-operation binary values, generic `-e`/`-E` controls on applicable commands, and opt-in anonymous referral chasing with DN/scope rewriting, control preservation, loop detection, and a five-hop default. Auth-only `-Y` supports PLAIN, CRAM-MD5, DIGEST-MD5, SCRAM-SHA-1/256/512, and EXTERNAL with `-U/-X/-R`, strict server proofs, and mutual TLS; OpenLDAP Compare/ldapexop differentials and pinned ldap-tools/libldap source contracts pass. GSSAPI, SCRAM-PLUS/channel binding, SASL security layers and interactive callbacks, generic Compare controls, complete referral URL/rebind behavior, and the full historical option surface remain |
| Load balancer tooling | partial | `ldap-go lloadd` parses standalone OpenLDAP-style configuration, validates the runnable subset and rejects unsupported settings with `-test-config`, serves multiple LDAP, LDAPS, StartTLS, and escaped-authority/three-slash LDAPI listeners, exposes listener/log and certificate/key overrides, maps config-file upstream LDAPS/StartTLS, service SASL, keepalive, and TCP user-timeout settings, and is exercised by unit, race, TLS topology, pinned-source, and live OpenLDAP 2.6.13 tests; PROXY listeners, the historical daemon option/signal/logging surface, embedded slapd-module mode, and runtime config/monitor administration remain |
