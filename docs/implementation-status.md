...
[compatibility.md](compatibility.md); an item is not supported until
...
checkout is never reset or switched. See [testing.md](testing.md) for
...
listed in [openldap-module-coverage.md](openldap-module-coverage.md).
...
`olcSockbufMaxIncoming` and `olcSockbufMaxIncomingAuth` apply OpenLDAP's
262,143/16,777,215-byte BER content defaults before allocating the request
body. Exact-boundary requests are accepted, overflow closes silently, and
successful, failed, anonymous, and repeated Bind transitions retain the
OpenLDAP connection-local limit snapshot. Online changes affect new
connections and existing connections at their next identity transition.
...
The contrib `noopsrch` overlay implements control
`1.3.6.1.4.1.4203.666.5.18` for database and frontend Search paths. It returns
no entries or references and reports the unrestricted entry/reference counts
plus the original size-limit result in its response control. Request-value and
duplicate-control validation and a dynamically built OpenLDAP module
differential cover its wire behavior.
...
command. With `olcGentleHUP: TRUE`, the first SIGHUP instead keeps existing
client sessions available for reads while rejecting subsequent update and
password-modify operations with `unwillingToPerform`; the process exits after
the last client disconnects. A second SIGHUP, SIGINT, or SIGTERM upgrades that
state to the bounded drain path.
...
# Add a local administration socket, or set -listen '' for LDAPI-only mode.
go run ./cmd/ldap-go serve \
  -db ./data/ldap-go.db \
  -listen '' \
...
## Bootstrap, AutoCA, and password policy
...
See [architecture.md](architecture.md) for the implementation model and
[testing.md](testing.md) for compatibility gates.
