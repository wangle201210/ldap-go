...
最新常用功能对照：2026-09-24 第四轮，100,000 个用户，基线 `aaf8350` 与最终
`current-sized` 对照，Apple M1 Pro，Go 禁用 cgo，原生服务为 OpenLDAP 2.6.13。
每行是三次重复测量的批次总耗时中位数，逐请求轮换服务端；仅计 SDK 调用时间，
结果校验不计时。双方均配置 uid/member/objectClass 等值索引。
相对性能为 `OpenLDAP / ldap-go × 100%`，越大越好。使用频率是常见认证/目录场景的
定性估计，不是实际流量统计。

| 常用功能 | 典型使用频率 | 次数 | ldap-go，默认权限 | OpenLDAP，默认权限 | 默认权限相对性能 | 显式 ACL 相对性能 |
| --- | --- | ---: | ---: | ---: | ---: | ---: |
| 用户 Bind，SSHA | 极高 | 1,000 | 86.14 ms | 68.59 ms | 79.6% | 77.6% |
| 非管理员 Base，热点用户 | 高 | 1,000 | 112.90 ms | 84.84 ms | 75.1% | 71.8% |
| 非管理员索引查询，热点用户 | 极高 | 1,000 | 114.47 ms | 87.58 ms | 76.5% | 70.2% |
| 直接所属组查询 | 高 | 100 | 17.02 ms | 10.66 ms | 62.7% | 52.5% |
| 读取千人成员组 | 中 | 100 | 84.49 ms | 80.66 ms | 95.5% | 92.8% |
| 嵌套组，客户端 BFS | 中～高 | 100 次遍历 | 53.03 ms | 36.51 ms | 68.8% | 65.1% |

[常用功能报告](docs/common-ldap-performance.md)包含分散用户查询、准确 ACL 配置、
原始样本和验证记录；[SDK 对照工具](internal/cmd/ldapcommonbench/README.md)
可对临时服务复跑。本轮相较 `aaf8350`，SSHA Bind 耗时下降 **7.6%–9.9%**，
普通用户查询下降 **3.3%–9.5%**，所属组查询下降 **6.3%–9.2%**，嵌套组查询下降
**6.3%–7.8%**。四项追平目标**尚未完成**。R4 首次尝试的快路径拒绝了 SDK 的正数
查询条数限制，其数据保留为诊断证据；最终实现支持原有条数限制，没有修改测试负载。
密码强度、两次认证快照检查和实时 ACL 判断保持不变。报告继续记录已有操作属性兼容性
缺口，并保留[第三轮归档](docs/common-ldap-performance-20260924-r3.md)。

本轮计时边界和组夹具与此前不同；
[历史完整对照](docs/performance-optimization-20260923-round13.md)继续保留写入、
分页、并发和内存数据。
