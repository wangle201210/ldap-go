...
以下结果于 2026-08-31 在 Apple M1 Pro 上完成：生成 100,000 条记录，双方
使用相同索引、相同 OpenLDAP 2.6.13 客户端并通过本机回环地址访问。耗时与
资源项均为越低越好。比值按 `OpenLDAP / ldap-go` 计算：大于 1 表示
ldap-go 占优，数值越大表示提升越多。

| 指标 | ldap-go | OpenLDAP | OpenLDAP / ldap-go |
| --- | ---: | ---: | ---: |
| 导入并建立索引 | 105,727 ms | 832,618 ms | 7.88 |
| 启动至就绪 | 272 ms | 96 ms | 0.35 |
| 索引查询，重复批次 | 720 ms | 608 ms | 0.84 |
| 索引查询，首批 | 1,953 ms | 617 ms | 0.32 |
| 无索引负查询，重复批次 | 30 ms | 338 ms | 11.27 |
| 无索引负查询，首批 | 1,016 ms | 390 ms | 0.38 |
| 分页全量遍历，重复批次 | 718 ms | 1,543 ms | 2.15 |
| 分页全量遍历，首次 | 2,012 ms | 715 ms | 0.36 |
| 并发索引查询 | 258 ms | 275 ms | 1.07 |
| Modify | 565 ms | 3,780 ms | 6.69 |
| 工作负载结束后的 RSS | 462.7 MiB | 93.6 MiB | 0.20 |
| 空闲 10 秒后的 RSS | 207.2 MiB | 89.7 MiB | 0.43 |
| 数据库文件 | 118.1 MiB | 81.3 MiB | 0.69 |
