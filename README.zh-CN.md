...
对照 OpenLDAP 2.6.13。每行按所列次数统计总耗时，取三个独立进程的中位数；
写入在准备数据及缓存预热后测量。相对性能为 `OpenLDAP / ldap-go × 100%`，
大于 100% 表示 ldap-go 占优。

| 指标 | 次数 | ldap-go | OpenLDAP | 相对性能 |
| --- | ---: | ---: | ---: | ---: |
| 普通用户 Bind，SSHA | 1,000 | 177.45 ms | 87.51 ms | 49.3% |
| 管理员 Bind | 1,000 | 108.26 ms | 73.57 ms | 68.0% |
| Base 查询 | 1,000 | 107.17 ms | 90.39 ms | 84.3% |
| 索引等值查询 | 1,000 | 121.64 ms | 102.25 ms | 84.1% |
| Compare，匹配 | 1,000 | 117.46 ms | 72.93 ms | 62.1% |
| 前缀子串查询 | 20 | 1,157.95 ms | 625.05 ms | 54.0% |
| Add | 20 | 19.29 ms | 115.50 ms | 598.8% |
| Modify，修改非索引 description | 20 | 10.26 ms | 116.25 ms | 1,133.2% |
| ModifyDN | 20 | 28.85 ms | 129.76 ms | 449.8% |
| Delete | 20 | 20.85 ms | 133.65 ms | 640.9% |

完整普通属性导出一致。[最新常用功能报告](docs/performance-optimization-20260923-round9.md)
记录了相对 `42bb528`，普通用户 Bind 耗时下降约 53%，Compare 约 19-23%。
Base/等值查询结果有波动，长批次包含变慢样本，不宣称稳定提升。读/认证负载后
RSS 为 473.4 / 95.2 MiB。启动、首次初始化、Bind、子串查询和内存仍有差距。
