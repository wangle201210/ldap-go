...
| --- | ---: | ---: | ---: | ---: |
| 普通用户 Bind，SSHA | 3,000 | 380.35 ms | 195.69 ms | 51.5% |
| 管理员 Bind | 3,000 | 269.17 ms | 194.37 ms | 72.2% |
| Base 查询 | 3,000 | 308.35 ms | 245.55 ms | 79.6% |
| 索引等值查询 | 3,000 | 321.94 ms | 268.46 ms | 83.4% |
| Compare，匹配 | 3,000 | 325.59 ms | 209.41 ms | 64.3% |
| 前缀子串查询 | 20 | 906.59 ms | 618.63 ms | 68.2% |
| 无匹配子串查询 | 20 | 900.94 ms | 621.25 ms | 69.0% |
| Add | 20 | 15.46 ms | 110.41 ms | 714.0% |
| Modify，修改非索引 description | 20 | 8.23 ms | 116.95 ms | 1,420.8% |
| ModifyDN | 20 | 28.39 ms | 111.31 ms | 392.0% |
| Delete | 20 | 18.60 ms | 117.94 ms | 634.2% |

完整普通属性导出一致。[最新报告](docs/performance-optimization-20260923-round12.md)
记录了相对 `9581b3d`，子串查询耗时下降约 18%。短操作仍有涨有跌，
首轮、批次交替及逐请求复测的数据均保留。首轮读/认证 RSS 为
414.2 / 95.6 MiB。小批量串行写入结果不能外推至所有生产负载。
