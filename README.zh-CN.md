...
| --- | ---: | ---: | ---: | ---: |
| 普通用户 Bind，SSHA | 1,000 | 132.30 ms | 69.13 ms | 52.2% |
| 管理员 Bind | 1,000 | 88.75 ms | 66.51 ms | 74.9% |
| Base 查询 | 1,000 | 103.91 ms | 84.25 ms | 81.1% |
| 索引等值查询 | 1,000 | 110.24 ms | 91.05 ms | 82.6% |
| Compare，匹配 | 1,000 | 109.18 ms | 73.21 ms | 67.0% |
| 前缀子串查询 | 20 | 1,123.83 ms | 620.02 ms | 55.2% |
| Add | 20 | 16.60 ms | 95.42 ms | 574.9% |
| Modify，修改非索引 description | 20 | 9.37 ms | 90.09 ms | 961.8% |
| ModifyDN | 20 | 27.93 ms | 102.22 ms | 366.0% |
| Delete | 20 | 18.09 ms | 92.74 ms | 512.7% |

完整普通属性导出一致。[最新报告](docs/performance-optimization-20260923-round10.md)
记录了相对 `fc58dd4`，普通用户登录耗时再降约 23%，管理员登录约 17%，
诊断场景的保留堆减少约 38%。读/认证负载后 RSS 为 442.5 / 145.2 MiB。
其他查询基本持平；报告保留了写入波动及大批次复测。启动、首次初始化、
Bind、子串查询和内存仍有差距。
