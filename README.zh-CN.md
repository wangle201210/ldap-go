...
资源项均为越低越好；比值小于 1 表示 ldap-go 占优。

| 指标 | ldap-go | OpenLDAP | ldap-go / OpenLDAP |
| --- | ---: | ---: | ---: |
| 导入并建立索引 | 105,727 ms | 832,618 ms | 0.13 |
| 启动至就绪 | 272 ms | 96 ms | 2.83 |
| 索引查询，重复批次 | 720 ms | 608 ms | 1.18 |
| 索引查询，首批 | 1,953 ms | 617 ms | 3.17 |
| 无索引负查询，重复批次 | 30 ms | 338 ms | 0.09 |
| 无索引负查询，首批 | 1,016 ms | 390 ms | 2.61 |
| 分页全量遍历，重复批次 | 718 ms | 1,543 ms | 0.47 |
| 分页全量遍历，首次 | 2,012 ms | 715 ms | 2.81 |
| 并发索引查询 | 258 ms | 275 ms | 0.94 |
| Modify | 565 ms | 3,780 ms | 0.15 |
| 工作负载结束后的 RSS | 462.7 MiB | 93.6 MiB | 4.94 |
| 空闲 10 秒后的 RSS | 207.2 MiB | 89.7 MiB | 2.31 |
| 数据库文件 | 118.1 MiB | 81.3 MiB | 1.45 |
