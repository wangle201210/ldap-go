...
最新对照：2026-09-24 第七轮，100,000 个用户，基线 `ac7182c` 与 `current`，
...
| --- | --- | ---: | ---: | ---: | ---: | ---: |
| 用户 Bind，SSHA | 极高 | 1,000 | 92.48 ms | 73.45 ms | 79.4% | 73.2% |
| 非管理员 Base，热点用户 | 高 | 1,000 | 121.80 ms | 94.79 ms | 77.8% | 73.5% |
| 非管理员索引查询，热点用户 | 极高 | 1,000 | 118.53 ms | 89.80 ms | 75.8% | 77.0% |
| 直接所属组查询 | 高 | 100 | 17.11 ms | 11.76 ms | 68.8% | 67.4% |
| 读取千人成员组 | 中 | 100 | 97.61 ms | 88.71 ms | 90.9% | 88.6% |
| 嵌套组，客户端 BFS | 中～高 | 100 次遍历 | 51.66 ms | 36.84 ms | 71.3% | 74.3% |

[R7 报告](docs/common-ldap-performance.md)保留两张完整测量表及所有较慢结果。
相较基线 `ac7182c`，SSHA Bind 在**显式 ACL 下慢 7.3%，默认权限下慢 0.7%**；显式 ACL 三个配对批次
分别慢 0.44%、0.26%、12.25%。共享主机测量不能确定这些差异由代码改动造成。
直接所属组查询中位数分别改善 4.2%、0.4%。
**与 OpenLDAP 持平的目标尚未达成，不能声称所有负载都加速。**

R7 避免重复构造 DN 显示文本及规范化文本。与“热 DN 缓存命中后重新渲染”相比，
直接复用热缓存文本少一次分配；这是组件结果，不是 SDK 延迟收益。
缓存边界和逐次校验保留，没有新增授权缓存。最终纯 Go 测试、vet 和 355 条
原生检查通过；R2 已有操作属性兼容缺口仍未解决。

首次初始化失败单独归档；两轮正式测量使用新夹具和 30 秒客户端超时，SDK 计时
边界不变。详见[证据索引](docs/evidence/common-performance-20260924-r7/README.md)、
[第六轮原文归档](docs/common-ldap-performance-20260924-r6.md)和
[SDK 对照工具](internal/cmd/ldapcommonbench/README.md)。本检查点不再安排基准或复查。
