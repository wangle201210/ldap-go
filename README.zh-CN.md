...
最新常用功能对照：2026-09-24 第二轮，100,000 个用户，Apple M1 Pro，Go 禁用 cgo，
对照 OpenLDAP 2.6.13。每行是三轮测试的批次总耗时中位数，逐请求轮换服务端；
仅计 SDK 调用时间，结果校验不计时。双方均配置 uid/member/objectClass 等值索引。
相对性能为 `OpenLDAP / ldap-go × 100%`，越大越好。

| 常用功能 | 次数 | ldap-go，默认权限 | OpenLDAP，默认权限 | 默认权限相对性能 | 显式 ACL 相对性能 |
| --- | ---: | ---: | ---: | ---: | ---: |
| 用户 Bind，SSHA | 1,000 | 104.34 ms | 73.18 ms | 70.1% | 69.5% |
| 非管理员 Base，热点用户 | 1,000 | 121.49 ms | 85.70 ms | 70.5% | 63.8% |
| 非管理员索引查询，热点用户 | 1,000 | 120.42 ms | 87.55 ms | 72.7% | 65.5% |
| 直接所属组查询 | 100 | 31.17 ms | 11.53 ms | 37.0% | 38.6% |
| 读取千人成员组 | 100 | 100.23 ms | 88.19 ms | 88.0% | 84.7% |
| 嵌套组，客户端 BFS | 100 次遍历 | 76.30 ms | 40.21 ms | 52.7% | 50.3% |

[常用功能报告](docs/common-ldap-performance.md)包含分散用户查询、准确 ACL 配置、
原始样本和验证记录；[SDK 对照工具](internal/cmd/ldapcommonbench/README.md)
可对临时服务复跑。本轮相较 `e07231f`，显式 ACL 下所属组查询耗时下降 **88.8%**，
千人成员组读取下降 **77.5%**，常用 Base/等值查询下降约 **15%–17%**。
四项追平目标**尚未完成**，所属组查询仍有明显差距。默认权限下部分项目有小幅波动；
密码强度和权限判断保持原有行为。报告同时记录了优化前已存在的操作属性兼容性缺口。
