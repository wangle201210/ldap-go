...
最新常用功能对照：2026-09-24 第三轮，100,000 个用户，Apple M1 Pro，Go 禁用 cgo，
...
| --- | --- | ---: | ---: | ---: | ---: | ---: |
| 用户 Bind，SSHA | 极高 | 1,000 | 97.00 ms | 73.34 ms | 75.6% | 73.6% |
| 非管理员 Base，热点用户 | 高 | 1,000 | 109.04 ms | 77.97 ms | 71.5% | 65.1% |
| 非管理员索引查询，热点用户 | 极高 | 1,000 | 113.73 ms | 82.29 ms | 72.4% | 66.4% |
| 直接所属组查询 | 高 | 100 | 18.76 ms | 10.94 ms | 58.3% | 53.1% |
| 读取千人成员组 | 中 | 100 | 95.20 ms | 85.52 ms | 89.8% | 84.2% |
| 嵌套组，客户端 BFS | 中～高 | 100 次遍历 | 61.55 ms | 39.45 ms | 64.1% | 60.6% |

[常用功能报告](docs/common-ldap-performance.md)包含分散用户查询、准确 ACL 配置、
原始样本和验证记录；[SDK 对照工具](internal/cmd/ldapcommonbench/README.md)
可对临时服务复跑。本轮相较 `a7c2941`，所属组查询耗时下降 **35%–42%**，
嵌套组查询下降 **17%–20%**，SSHA Bind 下降 **5.6%–7.1%**；普通用户查询改善较小。
四项追平目标**尚未完成**。千人组显式 ACL 首轮出现变慢，五轮专项复测未复现，
所有样本均保留在报告中。密码强度和权限判断保持原有行为；已有操作属性兼容性缺口仍有记录。
