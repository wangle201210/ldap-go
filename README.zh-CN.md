...
对照 OpenLDAP 2.6.13。每行是所列次数的总耗时中位数。Bind、Compare 和
非管理员查询使用每种实现一个预热进程、逐请求轮换的三批样本；其他行使用
三个独立进程。
...
| --- | ---: | ---: | ---: | ---: |
| 普通用户 Bind，SSHA | 3,000 | 356.48 ms | 235.93 ms | 66.2% |
| 管理员 Bind | 3,000 | 258.88 ms | 203.96 ms | 78.8% |
| 管理员 Base 查询 | 1,000 | 99.08 ms | 80.76 ms | 81.5% |
| 管理员索引等值查询 | 1,000 | 109.66 ms | 85.20 ms | 77.7% |
| 非管理员 Base 查询 | 3,000 | 701.26 ms | 316.92 ms | 45.2% |
| 非管理员索引等值查询 | 3,000 | 696.55 ms | 322.22 ms | 46.3% |
| 管理员 Compare，匹配 | 3,000 | 310.39 ms | 213.31 ms | 68.7% |
| 前缀子串查询 | 20 | 910.67 ms | 612.17 ms | 67.2% |
| 无匹配子串查询 | 20 | 911.04 ms | 610.64 ms | 67.0% |
| Add | 20 | 18.47 ms | 99.57 ms | 539.1% |
| Modify，修改非索引 description | 20 | 8.73 ms | 92.89 ms | 1,064.7% |
| ModifyDN | 20 | 25.98 ms | 104.78 ms | 403.3% |
| Delete | 20 | 18.36 ms | 96.58 ms | 525.9% |

完整普通属性导出一致。[最新报告](docs/performance-optimization-20260923-round13.md)
记录了相对 `03991cc`，普通认证耗时下降约 14%，非管理员单条查询约 27%。
管理员操作的涨跌及并发查询复测均保留。读/认证 RSS 为 431.4 / 115.5 MiB，
采样累计分配降低尚未体现为 RSS 下降。小批量串行写入结果不能外推至所有
生产负载。认证、非管理员查询、子串扫描和内存仍明显落后于 OpenLDAP。
